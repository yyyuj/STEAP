...
def get_gwas_links(
    pivot_corr_df: pd.DataFrame, pattern: str, gwas_name: str
) -> pd.DataFrame:
    """
    Finds all correlations between the input gwas_name and gwas part of
    the pattern.
    """
    gwas_links = pivot_corr_df[
...
def annotation_layer(
    ...
    for m in constants.METHODS:
        ...
        gcircle.ax.plot(x, y, alpha=1, color=axis_color, lw=0.5, zorder=8)
        for i_scale in [-1, 1, -0.5, 0.5]:
            gcircle.ax.plot(
                x, y + i_scale * scale, alpha=0.75, color=axis_color, lw=0.25, zorder=8
            )
        bottom += 2 * scale
...
def preprocessing(
    ...
    """
    pattern = "|".join([v for values in gwas_group_dict.values() for v in values])
    ...
    if gwas_name is not None:
        ...
        pivot_corr_df["pval"] = pivot_corr_df["pval"] < pthres
        pivot_corr_df.columns = [
            " ".join(col).strip().replace(" ", "_")
...
def plot(
    ...
    links, nodes, gwas_links = preprocessing(
        corr_df, gwas_group_dict, gwas_name, sign_threshold
    )
    ...
    links.apply(
        lambda row: chord_plot_pandas(
            gcircle, row, gwas_group_dict, corr_limit, bottom
        ),
        axis=1,
    )
