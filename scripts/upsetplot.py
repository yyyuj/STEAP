...
def add_count_to_group_dict(
    df: pd.DataFrame, gwas_group_dict: dict[str, list[str]]
) -> dict[str, list[str]]:
    """
...
def make_df_sliced(
    df: pd.DataFrame, gwas_group_dict: dict[str, list[str]], sign_threshold: int
) -> pd.DataFrame:
    """
...
def get_shared_celltypes(
    ...
    df_sliced = make_df_sliced(df, gwas_group_dict, sign_threshold)
    df_sliced = (
        df_sliced.groupby(["specificity_id", "annotation"])[["group", "gwas"]]
        .agg(set)
        .reset_index()
    )
